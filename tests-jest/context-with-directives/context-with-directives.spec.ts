...
describe('context-with-directives:real', () => {
  ...
  it('renders everything right', async () => {
    ...
    expect(ngMocks.find(fixture.debugElement, '.template').nativeElement.innerHTML).toContain(
      ' template w/ directive w/o binding '
    );

    // template1 should be rendered under .template1
    expect(ngMocks.find(fixture.debugElement, '.template1').nativeElement.innerHTML).toContain(
      ' template w/ directive w/ binding 1 '
    );

    // template2 should not be rendered
    expect(fixture.nativeElement.innerHTML).not.toContain(' template w/ directive w/ binding w/o render ');

    // unused ng-templates shouldn't be rendered at all
    expect(fixture.nativeElement.innerHTML).not.toContain(' template w/o directive w/o binding ');
    expect(fixture.nativeElement.innerHTML).not.toContain(' template w/o directive w/ binding ');
...
describe('context-with-directives:mock', () => {
  ...
  it('renders everything what is not template', () => {
    ...
    expect(fixture.nativeElement.innerHTML).not.toContain(' template ');
  });
});
