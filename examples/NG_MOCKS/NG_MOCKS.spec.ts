...
describe('NG_MOCKS:deep', () => {
  ...
  it('should contain mocks', inject([NG_MOCKS], (mocks: Map<any, any>) => {
    ...
    // keep
    const componentWeDontWantToMimic = mocks.get(ComponentWeDontWantToMimic);
    expect(componentWeDontWantToMimic).toBe(ComponentWeDontWantToMimic);
    const directiveWeDontWantToMimic = mocks.get(DirectiveWeDontWantToMimic);
    expect(directiveWeDontWantToMimic).toBe(DirectiveWeDontWantToMimic);
    const pipeWeDontWantToMimic = mocks.get(WeDontWantToMimicPipe);
    expect(pipeWeDontWantToMimic).toBe(WeDontWantToMimicPipe);
    const serviceWeDontWantToMimic = mocks.get(ServiceWeDontWantToMimic);
    expect(serviceWeDontWantToMimic).toBe(ServiceWeDontWantToMimic);
    const injectionTokenWeDontWantToMimic = mocks.get(INJECTION_TOKEN_WE_DONT_WANT_TO_MIMIC);
    expect(injectionTokenWeDontWantToMimic).toBe(INJECTION_TOKEN_WE_DONT_WANT_TO_MIMIC);
    ...
    // mimic
    const moduleWeWantToMimicBesidesMyModule = mocks.get(ModuleWeWantToMimicBesidesMyModule);
    expect(isMockedNgDefOf(moduleWeWantToMimicBesidesMyModule, ModuleWeWantToMimicBesidesMyModule, 'm')).toBeTruthy();
    const componentWeWantToMimic = mocks.get(ComponentWeWantToMimic);
    expect(isMockedNgDefOf(componentWeWantToMimic, ComponentWeWantToMimic, 'c')).toBeTruthy();
    const directiveWeWantToMimic = mocks.get(DirectiveWeWantToMimic);
    expect(isMockedNgDefOf(directiveWeWantToMimic, DirectiveWeWantToMimic, 'd')).toBeTruthy();
    const pipeWeWantToMimic = mocks.get(WeWantToMimicPipe);
    expect(isMockedNgDefOf(pipeWeWantToMimic, WeWantToMimicPipe, 'p')).toBeTruthy();
    ...
    expect(mocks.has(INJECTION_TOKEN_WE_WANT_TO_MIMIC)).toBeDefined();
    expect(mocks.get(INJECTION_TOKEN_WE_WANT_TO_MIMIC)).toBeUndefined();
