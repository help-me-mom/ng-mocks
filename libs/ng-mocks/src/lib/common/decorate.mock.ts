...
export default (mock: AnyType<any>, source: AnyType<any>, configInput: ngMocksMockConfig = {}): void => {
  ...
  if((mock as any)?.ɵmod?.declarations){
    (mock as any).ɵmod.declarations.filter(
      (x: any) => x?.decorators?.[0]?.args?.[0] && x?.decorators?.[0]?.args?.[0].standalone === undefined
    ).map(
      (x: any) => x.decorators[0].args[0].standalone = false);
  }
};
