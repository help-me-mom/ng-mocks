...
describe('create-runtime-inject-provider', () => {
  ...
  describe('standalone dependency policies', () => {
    ...
    for (const type of ['Component', 'Directive', 'Pipe']) {
      it(`leaves a deeply kept ${type} factory intact beside a shallow target`, () => {
        class RootService {
          public value(): string {
            return 'real';
          }
        ...
        const injector = {
          get: (service: new () => RootService): RootService =>
            new service(),
          onDestroy: () => undefined,
        };
        const shallowFactory = (): RootService =>
          injector.get(RootService);
        const shallowDefinition = {
          factory: shallowFactory,
          selectors: [],
          inputs: {},
          outputs: {},
        };
        const keptFactory = (): RootService => new RootService();
        const keptDefinition = {
          factory: keptFactory,
          selectors: [],
          inputs: {},
          outputs: {},
        ...
        coreDefineProperty(ShallowTarget, '__annotations__', [
          { ngMetadataName: 'Component', standalone: true },
        ]);
        coreDefineProperty(ShallowTarget, 'ɵcmp', shallowDefinition);
        coreDefineProperty(ShallowTarget, 'ɵfac', shallowFactory);
        ...
        coreDefineProperty(KeptDependency, 'ɵfac', keptFactory);

        const provider = createRuntimeInjectProvider(
          new Set([ShallowTarget, KeptDependency]),
          new Map([
            [ShallowTarget, { shallow: true }],
            [KeptDependency, {}],
          ]),
          [],
          true,
        ...
        provider.useFactory(injector, new Set())();

        expect(keptDefinition.factory).toBe(keptFactory);
        expect(keptDefinition.factory().value()).toBe('real');
        expect(shallowDefinition.factory).not.toBe(shallowFactory);
        expect(shallowDefinition.factory().value()).toBeUndefined();
      });
    }

    it('omits runtime interception for a standalone declaration with shallow disabled', () => {
      class KeptDependency {}
      coreDefineProperty(KeptDependency, '__annotations__', [
        { ngMetadataName: 'Directive', standalone: true },
      ]);

      expect(
        createRuntimeInjectProvider(
          new Set([KeptDependency]),
          new Map([[KeptDependency, { shallow: false }]]),
          [],
          true,
          new Set(),
        ),
    ...
    it('honors forced root mocks while retaining explicit service keeps', () => {
      class RootService {
        public value(): string {
          return 'real';
        }
      ...
      for (const service of [RootService, KeptService]) {
        coreDefineProperty(service, 'ɵprov', { providedIn: 'root' });
      }
      const injector = {
        get: <T>(service: new () => T): T => new service(),
        onDestroy: () => undefined,
      };
      const factory = () => ({
        mocked: injector.get(RootService),
        kept: injector.get(KeptService),
      });
      ...
      coreDefineProperty(KeptDependency, 'ɵdir', definition);
      coreDefineProperty(KeptDependency, 'ɵfac', factory);
      ngMocksUniverse.config.set(
        'ngMocksDepsResolution',
        new Map([[NG_MOCKS_ROOT_PROVIDERS, 'mock']]),
      );

      const provider = createRuntimeInjectProvider(
        new Set([KeptDependency, KeptService]),
        new Map([
          [KeptDependency, {}],
          [KeptService, {}],
        ]),
      ...
      expect(definition.factory).not.toBe(factory);
      expect(value.mocked.value()).toBeUndefined();
      expect(value.kept.value()).toBe('kept');
      expect(injector.get(RootService)).toBe(value.mocked);
    });

    for (const forceRootMocks of [false, true]) {
      it(`respects reflected root keeps in a shallow target with forceRootMocks=${forceRootMocks}`, () => {
        class SharedRoot {
          public value(): string {
            return 'shared';
          }
        ...
        for (const service of [SharedRoot, ExplicitKeep]) {
          coreDefineProperty(service, 'ɵprov', {
            providedIn: 'root',
          });
        }
        const injector = {
        ...
        const factory = () => ({
          shared: injector.get(SharedRoot),
          explicit: injector.get(ExplicitKeep),
        });
        const definition = { factory };
        class ShallowTarget {}
        coreDefineProperty(ShallowTarget, '__annotations__', [
        ...
        coreDefineProperty(ShallowTarget, 'ɵdir', definition);
        coreDefineProperty(ShallowTarget, 'ɵfac', factory);
        const skippedRoots = new Set([SharedRoot]);
        ngMocksUniverse.config.set('ngMocksDepsSkip', skippedRoots);
        ngMocksUniverse.config.set(
          'ngMocksDepsResolution',
          new Map(
            forceRootMocks ? [[NG_MOCKS_ROOT_PROVIDERS, 'mock']] : [],
          ),
        );

        const provider = createRuntimeInjectProvider(
          new Set([ShallowTarget, ExplicitKeep]),
          new Map([
            [ShallowTarget, { shallow: true }],
            [ExplicitKeep, {}],
          ]),
        ...
        // Build-time graph state is restored before Angular runs the initializer.
        skippedRoots.clear();
        provider.useFactory(injector, new Set())();
        const value = definition.factory();

        if (forceRootMocks) {
          expect(value.shared.value()).toBeUndefined();
        } else {
          expect(value.shared.value()).toBe('shared');
        }
        expect(value.explicit.value()).toBe('explicit');
      });
    }
  });
