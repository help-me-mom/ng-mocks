...
function Bundler() {
    ...
    function initialize(karmaConfig, bundlerOptions, logger) {
        ...
        validateSyntax = bundlerOptions.validateSyntax !== undefined ? bundlerOptions.validateSyntax : true;
    ...
    function flushQueue(start) {
        ...
        bundleQueue.forEach(function(queued) {
            projectModules[queued.filename] = addLoaderFunction(queued, true);
        });
        ...
        bundleQueue.forEach(function(queued) {
            queued.callback(projectModules[queued.filename]);
        });
    ...
    function writeBundleFile() {
        ...
        var bundle = "(function(global){" + os.EOL +
                    ...
                    createGlobals() +
                    createBundle(vendorModules) +
                    createBundle(projectModules) +
    ...
    function createBundle(modules) {
        ...
        Object.keys(modules).forEach(function(filename) {
            bundle += modules[filename];
        });
    ...
    function resolveRequiredNonModuleContent(moduleName, requiredModule) {
        ...
        if(!requiredModule.isTypescriptFile && !requiredModule.isTypingsFile) {
            ...
            vendorModules[requiredModule.filename] = addLoaderFunction(requiredModule, false);
        }
    }
