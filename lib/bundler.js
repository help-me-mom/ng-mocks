...
function Bundler(config) {
            ...
            queued.requiredModules.forEach(function(requiredModule) {
                if(!requiredModule.isTypescriptFile &&
                   !(requiredModule.isTypingsFile && !isNpmModule(requiredModule.moduleName))) {
                    requiredModule.indent = "";
                    requiredModule.stack = [];
    ...
    function addEntrypointFilename(filename) {
        if(config.bundlerOptions.entrypoints.test(filename) &&
           entrypointFilenames.indexOf(filename) === -1) {
    ...
    function createGlobalsAsync() {
        ...
        var globals = {
                ...
                requiredModules: [
                    { moduleName: "process/browser", indent: "", stack: [] },
                    { moduleName: "buffer/", indent: "", stack: [] }
                ]
            },
            dependencies = [
    ...
    function resolveModulesAsync(requiringModule, requiredModule) {
        ...
        requiredModule.stack.push(requiredModule.indent + requiringModule + " " + requiredModule.moduleName);
        ...
        return new Promise(function(resolve, reject) {
            ...
            browserResolve(requiredModule.moduleName, bopts, function(error, filename) {
                if(error) {
                    ...
                    requiredModule.stack.forEach(function(s){
                        stack += s + os.EOL;
                    });
                    reject(new Error("Unable to resolve module [" +
                        requiredModule.moduleName + "] from [" + requiringModule + "]" + os.EOL + stack));
                    return;
                }
           ...
           config.bundlerOptions.noParse.indexOf(requiredModule.moduleName) === -1) {
            ...
            moduleNames.forEach(function(moduleName) {
                if(config.bundlerOptions.exclude.indexOf(moduleName) !== -1) {
                    ...
                    var dependency = { moduleName: moduleName, indent: requiredModule.indent + "  ", stack: requiredModule.stack };
