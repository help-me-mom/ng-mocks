...
// var ast = recast.parse("module.exports = require('./lib/React'); /*require some stuff*/ while(true) { require('jasmine'); } var x = require('@angular/core'); require('style/scss'); function foo(x) { require('@types/jasmine'); } if(1){ var x = require('react'); }");
...
function walkSyntaxTree(node, requireExpressions) {
    ...
    if(node.program) {
        walkSyntaxTree(node.program, requireExpressions);
    }
