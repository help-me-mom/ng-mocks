function Preprocessor(bundler, compiler, sharedProcessedFiles) {
        ...
        try {
            ...
            compiler.compile(file, function(emitOutput){
                ...
                bundler.bundle(file, mappedSource, emitOutput.requiredModules, shouldAddLoader(), function(bundled){
                    ...
                    assert(bundled, file, emitOutput);
    ...
    function assert(bundled, file, emitOutput) {

        if(typeof bundled !== "string") {

            throw new Error("Expected a string, but got " +
                Object.prototype.toString.call(bundled) + " for " + file.originalPath + "\n" +
                "emitOutput: " + (emitOutput ? JSON.stringify(emitOutput, null, 3) : emitOutput));
        }
    }
