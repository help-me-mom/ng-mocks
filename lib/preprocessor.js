...
function createSourcemap(file, content, transpiledContent, sourceMapText){

    var result = transpiledContent,
        map,
        datauri;
    ...
    result = transpiledContent.replace("//# sourceMappingURL=module.js.map", ""); // TODO: Is there a compiler option to disable this?
...
var createPreprocessor = function(_config, helper, logger) {
    ...
    return function(content, file, done) {
        ...
        try {
            ...
            loadModules(file.path, transpileOutput.outputText);
            result = createSourcemap(file, content, transpileOutput.outputText, transpileOutput.sourceMapText);
            result = addLoaderFunction(file.path, result);
...
//var ast = esprima.parse("module.exports = require('./lib/React'); /*require some stuff*/ while(true) { require('jasmine'); } var x = require('@angular/core'); require('style/scss'); function foo(x) { require('@types/jasmine'); } if(1){ var x = require('react'); }");
        ...
        if(isArray(node.body)) {
            ...
            node.body.forEach(function(statement){
                ...
                if(statement.expression) {
                    ...
                    handleRequireExpression(statement.expression, requireExpressions);

                    if(statement.expression.right) {

                        if(statement.expression.right.body) {
                            walkSyntaxTree(statement.expression.right, requireExpressions);
                        }

                        handleRequireExpression(statement.expression.right, requireExpressions);
                    }
                }
                ...
                if (statement.declarations){
                    ...
                    statement.declarations.forEach(function(declaration){
                        ...
                        handleRequireExpression(declaration.init, requireExpressions);
                    });
                }

                if(statement.consequent) {

                    walkSyntaxTree(statement.consequent, requireExpressions);
                }
            });
        }
        else {

            walkSyntaxTree(node.body, requireExpressions);
        }
    }
}

function handleRequireExpression(expression, requireExpressions) {
...
function loadModules(requiringModule, requiringModuleSource, file) {

    var ast = esprima.parse(requiringModuleSource),
        requireExpressions = [];

    walkSyntaxTree(ast, requireExpressions);

    requireExpressions.forEach(function(requireParameter){

        try{
            var requiredModule = requireParameter.value,
                resolvedModule = resolveModule(requiringModule, requiredModule);

            if(file) {
                requiringModuleSource = requiringModuleSource.replace(requireParameter.value, resolvedModule.name);
            }

            requireParameter.value = resolvedModule.name;
            ...
            if(resolvedModule.loadContent && !modules[resolvedModule.name]){
                ...
                if(path.extname(resolvedModule.path) === ".js"){
                    ...
                    modules[resolvedModule.name] = addLoaderFunction(resolvedModule.name, generatedSource);
                }
                else {
    ...
    return file ? requiringModuleSource : escodegen.generate(ast);
}

function addLoaderFunction(moduleName, source) {

    return "window.__cjs_modules_root__ = '" + path.dirname(moduleName) + "';window.__cjs_module__ = window.__cjs_module__ || {}; window.__cjs_module__['" + path.normalize(moduleName) + "'] = function(require, module, exports) { " + source + os.EOL + "}";
}
...
function resolveModule(requiringModule, requiredModule) {
    ...
    if (isNpmModule(requiredModule)){
        ...
        var relativePath = path.dirname(requiringModule),
            ...
            isNpm = name.indexOf("node_modules") !== -1,
            fullpath = isNpm ? require.resolve(name) : resolveLocalModule(name);

        return {
            path: fullpath,
            name: name,
            loadContent: isNpm || path.extname(fullpath) !== ".js"
        };
    }
}
