function Preprocessor(compiler, nodeModulesLoader, sharedProcessedFiles) {
    ...
    function instrumentSource(coveragePreprocessor, done, file, result) {
        ...
        if(config.karmaTypescriptConfig && config.karmaTypescriptConfig.disableCodeCoverageInstrumentation || excludeFromCoverage.test(file.originalPath)) {
    ...
    var create = function(_config, helper, logger) {
        ...
        var karmaTypescriptConfig = config.karmaTypescriptConfig || {},
