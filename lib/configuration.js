function Configuration() {
    ...
    function initialize(config, logger) {
        ...
        self.basePath = karmaConfig.basePath;
    ...
    function configurePreprocessor() {
        ...
        self.coverageOptions.exclude = defaultTo(self.coverageOptions.exclude, /\.(d|spec|test)\.ts/);
    ...
    function assertReportsConfiguration() {

        Object.keys(self.reports).forEach(function(reportType) {

            var reportConfig = self.reports[reportType];

            if(isPlainObject(reportConfig) && (!reportConfig || !reportConfig.directory || ! reportConfig.filename)) {

                throw new Error("Invalid report configuration, expected " +
