...
Await `fixture.whenStable()` before asserting the updated input or rendered content. These examples use a
`TargetComponent` with a `value` input and the selector `app-target`:
...
expect(params.value).toEqual('updated');
expect(fixture.point.componentInstance.value).toEqual('updated');
```

This also applies to wrapper input writes when no params object is supplied and to custom-template bindings:

```ts
const params = { value: 'initial' };
const fixture = MockRender('<app-target [value]="value"></app-target>', params);
await fixture.whenStable();
...
For custom templates, `MockRender` observes the initial own enumerable params keys. `MockRenderFactory` observes its
explicit binding list, including keys initially missing from params; without a list, it observes each fixture's
initial own enumerable params keys. These bindings may feed component inputs or ordinary template expressions.
Assignments to unselected keys and changes inside nested objects do not themselves schedule a check; use
`fixture.detectChanges()` when the template reads those values. Adding a params key after fixture creation does
not extend the wrapper proxy. Read-only or nonconfigurable params retain their descriptor behavior and are not
instrumented for automatic notification.

Explicit `fixture.detectChanges()` remains supported in both zoned and zoneless tests. The automatic scheduling
described here applies to Angular 20.2+ with `provideZonelessChangeDetection()`, and to Angular 19's experimental
zoneless provider.

### ComponentRef.setInput and signal inputs

`fixture.componentRef` belongs to the wrapper component generated by
`MockRender`, whereas `fixture.point.componentInstance` is the rendered
component. The wrapper declares ordinary Angular inputs, including when the
corresponding input on the rendered component is a signal input.

This separation is intentional. The wrapper owns the binding value and the
rendered component owns its `InputSignal`, including its transform and other
signal metadata. The connection is the generated Angular template binding:

```text
ComponentRef.setInput -> wrapper input -> generated template binding -> InputSignal
```

Consequently, `setInput` updates the wrapper immediately. Call
`fixture.detectChanges()` to forward the new value through the generated
binding to the real signal input:
...
const fixture = MockRender(CounterComponent);

fixture.componentRef.setInput('count', 1);
fixture.detectChanges();

expect(fixture.point.componentInstance.count()).toEqual(1);
```

The wrapper input must be ordinary rather than another signal declaration.
Copying the signal declaration would create a second, unrelated signal node;
it could also run an input transform once on the wrapper and again on the
rendered component. With an ordinary wrapper input, Angular writes the binding
value to the wrapper and the real signal input processes it exactly once.

Input aliases use their public template names with `setInput`. For example,
an input declared with `{ alias: 'publicCount' }` is updated with
`fixture.componentRef.setInput('publicCount', value)`, while the value is read
from the real component through its property name, such as
`fixture.point.componentInstance.count()`.

The available wrapper inputs match the generated template: `MockRender(Component)`
binds all inputs, while `MockRender(Component, params)` binds only the input
names present in `params`.
